...
function resolveMultiSidebar(config, path, headers, depth) {
    ...
    Object.keys(config).forEach(function(key) {
        if (path.indexOf(key) > -1) {
            matchSidebar.push({key, index: path.indexOf(key)})
        }
    })
    const currentMatchSidebar = matchSidebar.length > 1 ? matchSidebar.reduce((o, p) => {
        if (o.index <= p.index) {
            return o
        } else {
            return p
    ...
    const paths = [path, currentMatchSidebar && currentMatchSidebar.key];
